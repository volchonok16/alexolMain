...
def meeting_invite_html(
    *,
    ...
    wash = "#f0f9fb"
    line = "#d5e8ee"
    lead_on_accent = "#d7f6f4"

    def meta_cell(label: str, value: str) -> str:
        return (
            f'<td width="50%" valign="top" bgcolor="{wash}" '
            f'style="width:50%;padding:14px 16px;background-color:{wash};'
            f'border:1px solid {line};border-radius:12px">'
            f'<p style="margin:0 0 6px;font-size:11px;letter-spacing:0.1em;text-transform:uppercase;'
            f'color:{accent};font-weight:700;font-family:{font}">{_html(label)}</p>'
            f'<p style="margin:0;font-size:15px;line-height:1.4;font-weight:600;color:{ink};'
            f'font-family:{font}">{_html(value)}</p>'
            "</td>"
        )
    ...
    while idx < len(rows):
        ...
        meta_html += (
            ...
            'width="100%" style="margin:0 0 10px"><tr>'
        )
        meta_html += meta_cell(*left)
        meta_html += '<td width="10" style="width:10px;font-size:0;line-height:0">&nbsp;</td>'
        meta_html += meta_cell(*right) if right else '<td width="50%" style="width:50%">&nbsp;</td>'
        meta_html += "</tr></table>"
        idx += 2
    ...
    if join_url:
        ...
        button = (
            '<table role="presentation" cellspacing="0" cellpadding="0" border="0" width="100%" style="margin:22px 0 10px">'
            "<tr>"
            f'<td align="center" bgcolor="{accent}" '
            f'style="background-color:{accent};border-radius:12px;padding:18px 28px;">'
            ...
            "</td></tr></table>"
            f'<p style="margin:0;font-size:12px;line-height:1.5;color:{mute};font-family:{font}">'
            "Если кнопка не открывается, скопируйте ссылку:<br/>"
            f'<a class="alexol-join-fallback" href="{href}" style="color:{accent};word-break:break-all">{href}</a></p>'
        )

    desc = ""
    if description and description.strip():
        desc = (
            f'<table role="presentation" cellspacing="0" cellpadding="0" border="0" width="100%" style="margin:18px 0 0">'
            "<tr>"
            f'<td bgcolor="{wash}" style="padding:14px 16px;background-color:{wash};'
            f'border:1px solid {line};border-radius:12px">'
            f'<p style="margin:0 0 6px;font-size:11px;letter-spacing:0.1em;text-transform:uppercase;'
            f'color:{accent};font-weight:700;font-family:{font}">Заметка</p>'
            f'<p style="margin:0;font-size:14px;line-height:1.55;color:#334155;font-family:{font}">'
            f"{_html(description.strip())}</p>"
            "</td></tr></table>"
        )

    return f"""
<table role="presentation" cellspacing="0" cellpadding="0" border="0" width="100%" bgcolor="#e7eef3" style="background-color:#e7eef3">
  <tr>
    <td align="center" style="padding:28px 12px">
      <table role="presentation" cellspacing="0" cellpadding="0" border="0" width="560" bgcolor="#ffffff" style="width:560px;max-width:560px;background-color:#ffffff;border-radius:18px;border:1px solid {line};overflow:hidden">
        <tr>
          <td bgcolor="{accent}" style="background-color:{accent};padding:28px 32px 26px;border-radius:18px 18px 0 0">
            <p style="margin:0 0 10px;font-size:11px;letter-spacing:0.16em;text-transform:uppercase;color:#ffffff;font-weight:700;font-family:{font}">{_html(badge)}</p>
            <p style="margin:0 0 8px;font-size:24px;line-height:1.28;font-weight:700;color:#ffffff;font-family:{font}">{_html(title or "Встреча")}</p>
            <p style="margin:0;font-size:15px;line-height:1.5;color:{lead_on_accent};font-family:{font}">{_html(lead)}</p>
          </td>
        </tr>
        <tr>
          <td bgcolor="#ffffff" style="padding:24px 28px 28px;font-family:{font};background-color:#ffffff">
            {meta_html}
            {button}
            {desc}
          </td>
