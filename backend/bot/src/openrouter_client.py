...
class OpenRouterClient:
    ...
    def _clean_ai_response(self, text: str) -> str:
        ...
        # Удаляем "случайные" иностранные слова, оставляя только короткие общеупотребимые аббревиатуры
        # (AI, IT, API и т.п.) и названия компаний/продуктов из латиницы не трогаем, если они <= 3 символов.
        def remove_foreign_words(t: str) -> str:
            allowed_short = {"ai", "it", "api", "sql", "ml", "ui", "ux", "vr", "ar"}
            # Удаляем латинские слова длиной 4+ символа, чтобы избежать "unlock", "menjadi" и т.п.
            def repl(match):
                word = match.group(0)
            ...
            return re.sub(r"\b[A-Za-z]{4,}\b", repl, t)
...
Отдельной последней строкой добавь подпись: Alexol
...
НЕ используй хэштеги (#тег1 и т.п.), только текст и подпись Alexol.
...
— Оставить заявку на сайте: alexol.io
